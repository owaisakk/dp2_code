...
//OWAIS
static float dsp_buffer_x[250], dsp_buffer_y[250], dsp_buffer_z[250];
static float averagex, averagey, averagez;
static int dsp_count;
...
#endif
#if DO_LOG	
			//OWAIS: Adding buffer of 250 values to collect 5 second data for DSP
			dsp_buffer_x[dsp_count] = frame.acc.x;
			dsp_buffer_y[dsp_count] = frame.acc.y;
			dsp_buffer_z[dsp_count] = frame.acc.z;
			
			dsp_count++;
			
			//If we have 250 values then
			if(dsp_count == 250){
				dsp_count = 0;
				copy_dsp_buffer(dsp_buffer_x, dsp_buffer_y, dsp_buffer_z);
			}
