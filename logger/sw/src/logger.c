...
static UINT write_len_owais;
static char filename[32];
static FATFS fs;
static FIL file;
static FIL file_owais;
static char char_owais[] = {'D', 'S', 'P', ' ', 'A', 'V', 'E', 'R', 'A', 'G', 'E', ' ', 'V', 'A', 'L', 'U', 'E', 'S', ' ', '*', '*', '*', '*', '*', '*', '*'};
static FRESULT res;
static FRESULT res_owais;
static uint32_t cycle_len;
static uint32_t cycle_lcm;
static uint32_t num_pages;
static int is_open = 0;

//OWAIS: To store the buffered values while writing
static float log_dspx_buffer[250], log_dspy_buffer[250], log_dspz_buffer[250];
static int dsp_write_flag = 0;
static float averagexyz[3];
	
	
int test[] = {0x55555555, 0xAAAAAAAA, 0x55555555, 0xAAAAAAAA};
...
void copy_dsp_buffer(float dspx[], float dspy[], float dspz[]){
	int i;
	
	for(i = 0; i < 250; i++){
		log_dspx_buffer[i] = dspx[i];
		log_dspy_buffer[i] = dspy[i];
		log_dspz_buffer[i] = dspz[i];
	}
	dsp_write_flag = 1;
}
...
const char *logger_init(const char *prefix, uint32_t calibrated){
	...
	res_owais = f_open(&file_owais, "DSP", FA_READ | FA_WRITE | FA_CREATE_ALWAYS);	

	if(res_owais != FR_OK){
		return NULL;
	}

	if(f_lseek(&file_owais, 0) != FR_OK)
	return NULL;
	
	if(f_write(&file_owais, (uint8_t *)&char_owais, owais_length, &write_len_owais) != FR_OK)
	return NULL;

	if(f_lseek(&file_owais, owais_length+4) != FR_OK)
	return NULL;
...
void logger_update(jb_frame_t const * frame){
	...
	//OWAIS: writing to my file
	if(dsp_write_flag){
		arm_mean_f32(log_dspx_buffer, 250, &averagexyz[0]);
		arm_mean_f32(log_dspy_buffer, 250, &averagexyz[1]);
		arm_mean_f32(log_dspz_buffer, 250, &averagexyz[2]);
		
// 		if(f_write(&file_owais, (uint8_t *)averagexyz, 12, &write_len_owais) != FR_OK){
// 			while(1);
// 		}
		
		if(f_write(&file_owais, (uint8_t *)test, 16, &write_len_owais) != FR_OK){
			while(1);
		}
